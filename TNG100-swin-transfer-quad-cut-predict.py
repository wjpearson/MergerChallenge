...
list_test_ds = tf.data.Dataset.list_files(path+'*'+extn)
test_ds = prepare_dataset(list_test_ds, BATCH_SIZE, test_image_count, True)
test_iter = iter(test_ds)
#dist_test_non_ds = strategy.experimental_distribute_dataset(test_ds)
...
model.load_weights('/mnt/home/wpearson/merger_challenge/saved_model_Q_cut/checkpoint')
...
for step in range(0, STEPS_PER_EPOCH):
    ...
    if predict is None:
        ...
        predict = np.vstack((predict,x_pred))

clas = np.argmax(predict, axis=1)
clas_onehot = np.zeros((len(clas),4), dtype=int)
for i in range(0, len(clas)):
    clas_onehot[i,clas[i]] = 1

from astropy.table import Table

#0 is non-merger, 1 is pre-merger, 2 is ongoing-merger, 3 is post-merger
table = Table()
table['ID'] = names
table['merger_type'] = clas

table['is_non_major_merger'] = clas_onehot[:,0]
table['is_major_merger'] = -1*(clas_onehot[:,0]-1)
table['is_pre_merger'] = clas_onehot[:,1]
table['is_ongoing_merger'] = clas_onehot[:,2]
table['is_post_merger'] = clas_onehot[:,3]

table['prob_non_merger'] = predict[:,0]
table['prob_major_merger'] = 1-predict[:,0]
table['prob_pre_merger'] = predict[:,1]
table['prob_ongoing_merger'] = predict[:,2]
table['prob_post_merger'] = predict[:,3]

table.write('TNG_WJPearson_quad.fits', overwrite=True)
