...
def get_label(file_path):
    ...
    #value = tf.strings.to_number(sub_name[6])
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    verbose=1,
    filepath='/mnt/home/wpearson/merger_challenge/saved_model_B_cut/checkpoint',
    save_weights_only=True,
    monitor='val_loss',
    mode='min',
    save_best_only=True)

history = model.fit(train_ds, validation_data=valid_ds, epochs=EPOCHS, verbose=2, callbacks=[model_checkpoint_callback])
...
result[["binary_accuracy", "val_binary_accuracy"]].plot(xlabel="epoch", ylabel="score", ax=ax[0])
result[["loss", "val_loss"]].plot(xlabel="epoch", ylabel="score", ax=ax[1])

fig.savefig("swin-transfer-cut.png")
